...
def record_sim_trades(signals_df, testing_signal_predictions, testing_probability_estimates, X_test_index):
    ...
    predictions_df[["Entry Price", "Entry Time", "Exit Price", "Exit Time", "volume"]] = signals_df[["Entry Price", "Entry Time", "Exit Price", "Exit Time", "volume"]]
    ...
    for j in range(1, predictions_df.shape[0]):
        ...
        volume = predictions_df["volume"].iloc[j]
        ...
        if entry_time <= last_trade_exit_time or exit_signal == -1 or volume == 0:
            continue
        else:

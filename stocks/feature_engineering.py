...
def add_features(signals_df, reward, risk):
    ...
    for j in range(longest_MA_window, num_rows_in_df):
        # make an index_label to allow using the fast method .at
        index_label_j = signals_df.index[j]
        # entries will be on candle close
        entry = signals_df.at[index_label_j, "close"]
        signals_df.at[index_label_j, "Entry Price"] = entry
        signals_df.at[index_label_j, "Entry Time"] = signals_df.index[j]
        # calculate volatility for each candle
        atr = signals_df.at[index_label_j, "ATR"]
        ...
        for k in range(j + 1, num_rows_in_df):
            # make an index_label to allow using the fast method .at
            index_label_k = signals_df.index[k]
            # current low of the candle
            curr_low = signals_df.at[index_label_k, "low"]
            # current high of the candle
            curr_high = signals_df.at[index_label_k, "high"]
            ...
            if curr_low <= stop:
                signals_df.at[index_label_j, "Exit Price"] = stop
                signals_df.at[index_label_j, "Exit"] = -1
                # record exit time
                signals_df.at[index_label_j, "Exit Time"] = signals_df.index[k]
            ...
            # if current high breaks our target we should've sold: +1 in our "Exit" column
                ...
                signals_df.at[index_label_j, "Exit Price"] = target
                signals_df.at[index_label_j, "Exit"] = 1
                # record exit time
                signals_df.at[index_label_j, "Exit Time"] = signals_df.index[k]
...
# Define a function to add features to a single chunk of data
def add_features_chunk(chunk, reward, risk):
    # Same content as the original add_features function, but applied to a single chunk
    return add_features(chunk, reward, risk)